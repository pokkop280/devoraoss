...
import { showDevoraLoginOverlay } from './devoraLoginOverlay.js';
...
export class GlobalCompositeBar extends Disposable {
	...
	getContextMenuActions(): IAction[] {
		return [toAction({ id: 'toggleAccountsVisibility', label: localize('accounts', "Devora Account"), checked: this.accountsVisibilityPreference, run: () => this.accountsVisibilityPreference = !this.accountsVisibilityPreference })];
	}
...
abstract class AbstractGlobalActivityActionViewItem extends CompositeBarActionViewItem {
	...
	private async run(): Promise<void> {
		...
		let actions = await this.resolveMainMenuActions(menu, disposables);
		...
		// Fallback: if the menu resolved empty (no extensions registered into GlobalActivity),
		// show a minimal Devora manage menu so the button always responds.
		if (actions.length === 0) {
			actions = [
				toAction({ id: 'workbench.action.openSettings', label: localize('settings', 'Settings'), run: () => this.commandService.executeCommand('workbench.action.openSettings') }),
				toAction({ id: 'workbench.action.openGlobalKeybindings', label: localize('keybindings', 'Keyboard Shortcuts'), run: () => this.commandService.executeCommand('workbench.action.openGlobalKeybindings') }),
				toAction({ id: 'workbench.action.selectTheme', label: localize('colorTheme', 'Color Theme'), run: () => this.commandService.executeCommand('workbench.action.selectTheme') }),
				new Separator(),
				toAction({ id: 'workbench.action.showCommands', label: localize('commandPalette2', 'Command Palette...'), run: () => this.commandService.executeCommand('workbench.action.showCommands') }),
			];
		}
...
export class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
	...
	private async doInitialize(): Promise<void> {
		...
		// Error logging removed since initialization variables and properties are simplified
	}

	//#region overrides

	/** Override: show Devora login overlay on left-click when no account is signed in. */
	protected override async resolveMainMenuActions(accountsMenu: IMenu, disposables: DisposableStore): Promise<IAction[]> {
		// ALWAYS show the Devora custom login overlay when the profile icon is left-clicked
		showDevoraLoginOverlay();
		return []; // return empty so the context menu won't appear
	}
